...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  const { _hasHydrated, accessToken, refreshToken, expires, user, isAdmin, logout } = useAuthStore(
    useShallow(state => ({
  ...
  // 页面加载/刷新时，若 access token 已过期，主动刷新（避免加载 stale 界面）
  useEffect(() => {
    ...
    if (accessToken && expires && Date.now() >= Number(expires) && refreshToken) {
      ...
      refreshAccessToken()
        .catch(() => {
          // 刷新失败：清登录态 + 跳登录（handleUnauthorized 也会触发，这里兜底）
          logout();
          router.replace("/login");
        })
        .finally(() => setValidating(false));
    }
  }, [_hasHydrated, accessToken, expires, refreshToken, logout, router]);
