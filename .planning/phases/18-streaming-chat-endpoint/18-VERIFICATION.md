...
| ChatWindow with useChat streaming | ✅ PASS | `frontend/src/components/chat/ChatWindow.tsx` — `useChat` imported |
...
| conversationId NOT generated by frontend (D-373) | ✅ PASS | `ChatWindow.tsx` does NOT send conversationId — backend creates Sqids IDs |

### Plan 18-03: Unit tests

| Must-Have | Status | Evidence |
|-----------|--------|----------|
| tool-bridge spec: 4+ test cases | ✅ PASS | `server/src/ai/tools/tool-bridge.spec.ts` — 5 `it()` cases (incl. duplicate name check) |
| ChatService spec: 4+ test cases | ✅ PASS | `server/src/ai/chat.service.spec.ts` — 7 `it()` cases (incl. conversationId validation, onFinish error logging) |
| Controller spec: 4+ test cases | ✅ PASS | `server/src/ai/ai-chat.controller.spec.ts` — 4 `it()` cases |
| All tests pass | ✅ PASS | `cd server && npx vitest run src/ai/` — 109 passed, 0 failed |
...
- **Total must-haves:** 17
- **Verified:** 17/17 (100%)
- **Automated checks:** 17 PASS, 0 FAIL
- **Human verification items:** 5 (require browser testing)
- **Test suite:** 109 AI tests passed, 0 failed
- **Code review fixes:** 5 (conversationId UUID→Sqids, onFinish error logging, decodePublicID validation, ModuleRef mock, duplicate tool name check)
