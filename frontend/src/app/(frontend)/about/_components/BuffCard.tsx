/*
 * @Author: 安知鱼
 * @Date: 2026-02-12 15:36:10
 * @Description:
 * @LastEditTime: 2026-02-12 16:34:42
 * @LastEditors: 安知鱼
 */
"use client";
...
function DiceD20Icon({ style }: { style?: React.CSSProperties }) {
  return (
    <svg xmlns="http://www.w3.org/2000/svg" width="480" height="512" viewBox="0 0 480 512" style={style}>
      <path
        fill="currentColor"
...
export function BuffCard({ buff }: BuffCardProps) {
  return (
    <div className={`${styles.item} ${styles.buff}`}>
      <div className={styles.cardContent} style={{ justifyContent: "center" }}>
        <div className={styles.itemTips}>{buff.tips}</div>
        <div className={styles.buffTitle}>
          <span style={{ marginBottom: 8, opacity: 0.6 }}>{buff.top}</span>
          <span>{buff.bottom}</span>
        </div>
      </div>
      <div className={styles.buffBgIcon}>
        <DiceD20Icon style={{ width: 192, height: 192 }} />
      </div>
    </div>
  );
}
